...
// SetupInput holds parameters for the swiftui-plus setup command.
...
type templateFile struct {
	templateName string
	outputFile   string // relative to Sources/SwiftUIPlus/
}

var sourceFiles = []templateFile{
	{templateName: "swiftui_plus.swift.tmpl", outputFile: "SwiftUIPlus.swift"},
	{templateName: "async_button.swift.tmpl", outputFile: "Components/AsyncButton.swift"},
}

// Setup creates the SwiftUIPlus module with UI components.
func Setup(input SetupInput) error {
	...
	for _, tf := range sourceFiles {
		...
		if err := os.MkdirAll(outputDir, 0o755); err != nil {
			return fmt.Errorf("create directory for %s: %w", tf.outputFile, err)
		}
...
func addModuleToProjectSwift(projectSwiftPath string) error {
	err := tuistproj.ApplyManifestEditsToFile(projectSwiftPath, tuistproj.ManifestEdit{
		...
		Name:    moduleName,
		Content: fmt.Sprintf(`.external(name: "%s")`, moduleName),
	})
...
func addModuleToRootPackageSwift(rootPackageSwiftPath, modulesRelPath string) error {
	refPath := filepath.ToSlash(filepath.Join(modulesRelPath, moduleName))
