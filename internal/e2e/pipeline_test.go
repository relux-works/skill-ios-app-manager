...
func verifyTodoListModule(t *testing.T, projectRoot string, modulesPath string) {
	...
	requireFile(t, filepath.Join(interfaceSources, "Namespace.swift"))
	requireFile(t, filepath.Join(interfaceSources, "Module.swift"))
	requireFile(t, filepath.Join(interfaceSources, "Module+Interface.swift"))

	requireFile(t, filepath.Join(implSources, "Module+Impl.swift"))
}
