...
`)
	...
	if strings.Contains(projectManifest, `"UISupportedInterfaceOrientations~ipad":`) {
		t.Fatalf("Project.swift kept stale iPad orientation:\n%s", projectManifest)
	}
...
func TestSyncPresentationConfigExplicitPlatformsUseSeparateOrientationKeys(t *testing.T) {
	t.Parallel()
