...
func TestScaffoldFilesRuntimeProfilesConvergesAndRestoresLegacy(t *testing.T) {
	t.Parallel()

	outputDir := filepath.Join(t.TempDir(), "AppConfig")
	if err := scaffoldFiles(outputDir); err != nil {
		t.Fatalf("legacy scaffoldFiles() error = %v", err)
	}
	if err := scaffoldFiles(outputDir, true); err != nil {
		t.Fatalf("runtime scaffoldFiles() error = %v", err)
	}

	managerPath := filepath.Join(outputDir, "AppConfig.Manager.swift")
	manager, err := os.ReadFile(managerPath)
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
		"Generated by ios-app-manager runtime-profiles",
		...
		"showsPersistentEnvironmentMarker",
	} {
		if !strings.Contains(string(manager), want) {
			t.Fatalf("runtime AppConfig.Manager.swift missing %q:\n%s", want, manager)
		}
	}
	if strings.Contains(string(manager), "/api/v1") {
		t.Fatalf("runtime AppConfig.Manager.swift generated an arbitrary API path:\n%s", manager)
	}

	configuration, err := os.ReadFile(filepath.Join(outputDir, "AppConfig.Env+Configuration.swift"))
	if err != nil {
		t.Fatal(err)
	}
	for _, want := range []string{
		"public let descriptor: BackendEnvironmentDescriptor",
		"descriptor.apiOrigin.absoluteString",
		"public var apiOrigin: URL",
	} {
		if !strings.Contains(string(configuration), want) {
			t.Fatalf("runtime AppConfig configuration missing %q:\n%s", want, configuration)
		}
	}

	if err := scaffoldFiles(outputDir, true); err != nil {
		t.Fatalf("second runtime scaffoldFiles() error = %v", err)
	}
	secondManager, err := os.ReadFile(managerPath)
	if err != nil {
		t.Fatal(err)
	}
	if string(secondManager) != string(manager) {
		t.Fatal("second runtime scaffoldFiles() did not converge")
	}

	if err := scaffoldFiles(outputDir, false); err != nil {
		t.Fatalf("legacy restoration scaffoldFiles() error = %v", err)
	}
	restored, err := os.ReadFile(managerPath)
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(restored), "GeneratedRuntimeProfiles") || strings.Contains(string(restored), "runtime-profiles") {
		t.Fatalf("restored AppConfig.Manager.swift retained runtime integration:\n%s", restored)
	}
	if !strings.Contains(string(restored), "return .prod") {
		t.Fatalf("restored AppConfig.Manager.swift missing legacy behavior:\n%s", restored)
	}
}

func TestScaffoldFilesRuntimeProfilesRejectsCustomManagerMigration(t *testing.T) {
	t.Parallel()
	...
	managerPath := filepath.Join(outputDir, "AppConfig.Manager.swift")
	if err := os.WriteFile(managerPath, []byte("// custom manager\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	err := scaffoldFiles(outputDir, true)
	if err == nil || !strings.Contains(err.Error(), "contains custom AppConfig behavior") {
		t.Fatalf("scaffoldFiles() error = %v, want safe custom migration refusal", err)
	}
	content, readErr := os.ReadFile(managerPath)
	if readErr != nil {
		t.Fatal(readErr)
	}
	if string(content) != "// custom manager\n" {
		t.Fatalf("custom manager was overwritten:\n%s", content)
	}
}
