...
- Synchronize Tuist and package min target: `ios-app-manager generate min-target`
...
- `generate min-target` depends on the same scaffold shape and synchronizes `min_target` across every generated host/test/extension Tuist target, existing root `Package.swift` `IPHONEOS_DEPLOYMENT_TARGET` overrides, and first-party local-package iOS `platforms` declarations recursively discovered below `modules_path`. Local package declarations converge to PackageDescription-compatible `.iOS("<major>.<minor>")` string syntax, accepting older enum declarations as input. It validates all target layouts and writability before writing, so unsupported declarations and partial layouts do not leave a partial migration.
...
- `generate runtime-profiles` owns typed distribution/backend descriptors, fail-closed Firebase public-input validation, explicit public-identity sharing groups, Tuist configurations/schemes, executable Tests actions, and policy-aware AppConfig integration. `runtime_profiles.test_action.targets` must map at least one existing test target; optional static non-secret `launch_arguments` are emitted only for the Tests action. Read [`references/runtime-profiles.md`](references/runtime-profiles.md) before adding or changing runtime-profile config.
