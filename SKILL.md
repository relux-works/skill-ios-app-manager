...
- Projects may configure lifecycle scripts in `ios-app-manager.json` under `scripts.pre_generate`. Each script node has `path`, `language`, and optional `description`; paths are relative to the project root and cannot escape it. Supported `language` values are `bash`, `swift`, `go`, and `executable`. Generated Makefiles run these scripts after `tuist install` and before `tuist generate`, which is the safe point for patching SwiftPM checkouts before Tuist converts them into Xcode projects.
