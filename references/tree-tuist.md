...
- Currently syncs version markers, min deployment targets across Tuist targets plus root/local Package.swift declarations, host app capabilities from `app_groups`, the app target `UIBackgroundModes` entry from `background_modes`, strict Swift compiler build flags, and package strictness.
